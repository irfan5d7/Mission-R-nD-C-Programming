...
void task2() {
	...
	invokePrint2(&printMessage, "Irfan");
}
...
char * subString(char *str, int a, int b){
	if (a > 0 && b > 0 && a < b)
	{
		char *substr = (char *)malloc(sizeof(char)*((b - a) + 1));
		int i, k = 0;
		for (i = a - 1; i < b; i++)
		{
			substr[k] = str[i];
...
void task3() {
	...
	char *subst= invokeSubStr(&subString, "Mission", 2, 5);
	printf("Substring is %s", subst);
}
