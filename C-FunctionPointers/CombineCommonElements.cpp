...
struct ObjectNode ** combine_common_elements(struct ObjectNode ** objectNodes1, struct ObjectNode ** objectNodes2, int len1, int len2, int *commonElementsLength)
{
	...
	int i, j, k=0,chk,alen=0;
	int *indxarr = (int *)calloc(sizeof(int), len1);
	*(commonElementsLength) = 0;
	for (i = 0; i < len1; i++)
	{
		for (j = 0; j < len2; j++)
		{
			chk = objectNodes1[i]->equals(objectNodes1[i]->value, objectNodes2[j]->value);
			if (chk == 1)
			{
				indxarr[k] = i;
				k++;
	...
	for (i = 0; i < alen; i++)
	{
		for (j = i + 1; j < alen; j++)
		{
			if (indxarr[i] >= 0 && indxarr[j] >= 0)
			{
				chk = equalsInt1(objectNodes1[indxarr[i]]->value, objectNodes1[indxarr[j]]->value);
				if (chk == 0)
				{
					chk = equalsFloat1(objectNodes1[indxarr[i]]->value, objectNodes1[indxarr[j]]->value);
					if (chk != 0)
					{
						indxarr[j] = -1;
					}
	...
	for (i = 0; i <alen; i++)
	{
		if (indxarr[i] >= 0)
		{
			result[k] = objectNodes1[indxarr[i]];
			k++;
